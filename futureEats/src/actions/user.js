...
export const addAddress = (body) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      "https://us-central1-missao-newton.cloudfunctions.net/futureEats/address",
      body,
      {
        headers: {
          auth: token,
...
export const getFullAddress = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://us-central1-missao-newton.cloudfunctions.net/futureEats/profile/address",
      {
        headers: {
          auth: token,
        },
...
export const getProfile = (token) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://us-central1-missao-newton.cloudfunctions.net/futureEats/profile",
      {
        headers: {
          auth: token,
        },
