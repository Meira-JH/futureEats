...
export class PageCart extends React.Component {
  ...
  handlePlaceOrder = () => {
    const ordersToPlace = {
      products: this.props.orders.map((order) => ({
        id: order.id,
      ...
      <CartWrapper>
        ...
        <DeliveryAddressWrapper>
          ...
          <Address>
            {userAddress ? (
              <span>
                {userAddress.neighbourhood}, {userAddress.number},{" "}
                {userAddress.street}{" "}
              </span>
            ) : (
        ...
        <PaymentWrapper orders={orders[0]}>
          ...
          <PaymentMethod>
            ...
            >
              <RadioPayment
                control={<RadioCustom color="secondary" />}
                value="money"
                label="Dinheiro"
              />
              <RadioPayment
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    placeOrder: (ordersToPlace, restaurantId) =>
      dispatch(placeOrder(ordersToPlace, restaurantId)),
  };
};
